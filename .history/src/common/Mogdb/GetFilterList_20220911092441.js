...
async function FilterList(data, filters, res) {
    const DataArr = [];
    let index = 0;
    let indexfiltermap = 0;
    const item = data[index];
    function CombackHell() {
        res.json(DataArr)
    }
    function dequy() {
        console.log("index", index)
        if (index === data.length - 1 && indexfiltermap === filters.length - 1) {
            CombackHell()
        }
        filters.map(async (filter, indexfilter) => {
            console.log("indexfilter", indexfilter)
            indexfiltermap = indexfilter;
            if (filter.isArray) {
                await filter.Modal.find({ "_id": { "$in": item[filter.variable] } }, (err, data) => {
                    if (indexfilter === filters.length - 1) {
                        index++;
                        DataArr.push({
                            ...item._doc,
